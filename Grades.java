...
class GradeComparator implements Comparator<String[]> {
    ...
    List<String> baseGrades = Arrays.asList(new String[] { "A", "B", "C", "D", "E", "FX", "F" });
    ...
    public int compare(String[] stud1, String[] stud2) {
        ...
        if (gv1 == gv2)
            return stud1[0].compareToIgnoreCase(stud2[0]) * -1;
        return gv1 > gv2 ? 1 : -1;
    }
    ...
    double gradeValue(String gradeString) {
        ...
        double extra = parts.length == 2 ? Math.log10(parts[1].length() + 1) * (parts[1].startsWith("+") ? 1 : -1) : 0;
